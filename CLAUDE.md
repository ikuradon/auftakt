...
├── backends/       # ストレージバックエンド（memory, dexie, cached）
...
```
0. 構造バリデーション → rejected
1. Ephemeral → reject
1.5 backend.isDeleted() → reject（永続化された削除レコードを確認）
2. Duplicate → seenOn更新
3. NIP-40期限切れ → reject
4. Kind 5 → 削除処理（backend.markDeleted / markReplaceDeletion で永続化）
5. Replaceable → created_at比較 → 置換 → isDeleted確認
6. Addressable → (kind,pubkey,d-tag)比較 → 置換 → isDeleted + getReplaceDeletion確認
7. Regular → 保存
8. backend.isDeleted確認（到着順序の逆転に対応）
...
| `src/backends/dexie.ts` | Dexie.js v4（strfry風スキーマ、クエリヒューリスティック、削除永続化） |
