...
@ikuradon/auftakt
├── core/
│   ├── store.ts              # NostrEventStore本体（add, query, getSync, fetchById, changes$）
│   ├── nip-rules.ts          # Replaceable/Addressable/Deletion/Expiration処理
│   ├── filter-matcher.ts     # Nostrフィルタとイベントのマッチング
│   ├── query-manager.ts      # Reactive query管理（逆引きインデックス、差分更新、マイクロバッチング）
│   ├── negative-cache.ts     # ネガティブキャッシュ（TTL付き）
│   └── snapshot.ts           # localStorageスナップショット（saveSnapshot/loadSnapshot）
├── backends/
│   ├── interface.ts          # StorageBackendインターフェース
│   ├── memory.ts             # メモリ実装（LRU eviction、kind別バジェット）
│   ├── indexeddb.ts          # IndexedDB実装（バッチ書き込み、ObjectStore分離、SSRフォールバック）
│   └── cached.ts             # Read-throughキャッシュラッパー（遅延ハイドレーション）
├── sync/
│   ├── synced-query.ts       # createSyncedQuery（REQライフサイクル、REQ重複排除、cache-aware since）
│   ├── global-feed.ts        # connectStore（reconcileDeletions統合、フィルタ不一致警告）
│   ├── deletion-reconcile.ts # 起動時Kind 5整合性チェック
│   ├── since-tracker.ts      # cache-aware since自動調整
│   └── publish.ts            # publishEvent（optimistic update）
└── adapters/
    └── svelte.ts             # Svelte 5 readable store（toReadable, createSvelteQuery）
```

> **Note:** `adapters/rxjs.ts` はspecの初版に記載されていたが、`store.query()` が既に `Observable<CachedEvent[]>` を返すため不要と判断し削除。
...
const store = createEventStore({
  ...
  // or: memoryBackend()
  // or: memoryBackend({ maxEvents: 10000, eviction: { strategy: 'lru' } })
  indexedTags: ['e', 'p', 't', 'a', 'k'], // オプション: インデックス対象タグを制限（デフォルト: 全タグ、NIP-01準拠）
});
```
...
```typescript
// 非リアクティブなスナップショット取得（TanStack Query等のqueryFn用）
const profiles = await store.getSync({ kinds: [0], authors: ['pk1'] });
```

`store.query()` がリアクティブObservableを返すのに対し、`getSync()` はPromiseで1回だけ結果を返す。TanStack Queryの `queryFn` やワンショット取得に使用。
...
IndexedDB前段にメモリLRUキャッシュを置く。write-through（put時に両方書く）、read-through（get時にメモリ優先→ミスならIDB→メモリに載せる）。query()は常にIDBに問い合わせ、結果をメモリに載せる。遅延ハイドレーションの効果: 起動時にIDB全件読み込みを行わず、アクセスされたイベントだけがメモリに溜まる。

### 6.4 localStorageスナップショット
...
await saveSnapshot(store, {
  ...
  filter: { kinds: [0], authors: followList }, // 保存対象
});

// 起動時に復元（同期的にlocalStorageから読み、Storeにadd）
const count = await loadSnapshot(store, { key: 'auftakt-snapshot' });
```

IndexedDBは非同期だが、localStorageは同期。最初のpaintに間に合わせるために、前回セッションの重要データ（プロフィール等）をlocalStorageに保存し、起動時に即復元する。
...
1. ~~**タグインデックスの粒度**~~ → ✅ `indexedTags` オプションで設定可能。デフォルト: 全タグ（NIP-01準拠）
2. ~~**Svelteアダプターの具体的API**~~ → ✅ `adapters/svelte.ts` で `toReadable()` / `createSvelteQuery()` を提供
3. ~~**テスト戦略**~~ → ✅ vitest + fake-indexeddb + @ikuradon/tsunagiya（リレーモック統合テスト）
4. **パッケージリポジトリ** — 独立リポジトリとして`@ikuradon/auftakt`で公開（npm publish未実施）
...
| 2026-03-30 | 実装完了後のspec更新。パッケージ構成を実装に合わせて更新（filter-matcher, query-manager, snapshot, cached, interface, publish追加。rxjsアダプター削除）。§4.3.1 getSync() API追加。§6.3 cachedBackend追加。§6.4 localStorageスナップショット追加。§4.1 indexedTagsオプション追加。§13の解決済み項目を更新 |
